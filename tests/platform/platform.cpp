...
TEST(PlatformTest, IsZeroTest) { ASSERT_TRUE(IsZero(0.)); }
...
TEST(PlatformTest, CubeTest) { ASSERT_DOUBLE_EQ(Cube(2.0), 8.0); }

TEST(PlatformTest, MaxTest) { ASSERT_DOUBLE_EQ(Max(2.0, 0.), 2.0); }

TEST(PlatformTest, MinTest) { ASSERT_DOUBLE_EQ(Min(2.0, dal::INFINITY), 2.); }
