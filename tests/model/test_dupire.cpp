...
using namespace Dal;
using Dal::AAD::DupireModelData_;
...
TEST(ModelTest, TestDupireModelData) {
    ...
    ASSERT_NEAR(std::dynamic_pointer_cast<const AAD::DupireModelData_>(rtn)->spot_, 100.0, 1e-8);
    ASSERT_NEAR(std::dynamic_pointer_cast<const AAD::DupireModelData_>(rtn)->rate_, 0.05, 1e-8);
}
