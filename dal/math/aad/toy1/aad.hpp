...
namespace Dal {
    namespace AAD_Toy1 {
        ...
        public:
            ...
            template <class V_> void PostOrder(V_& visitFunc) {
                if (!processed_) {
            ...
            template <class V_> void PreOrder(V_& visitFunc) {
                visitFunc(*this);
                for (const auto& arg : arguments_)
                    arg->PreOrder(visitFunc);
            }

            template <class V_> void BreadthFirst(V_& visitFunc) {
                std::queue<std::shared_ptr<Node_>> q;
                BreadthFirst(visitFunc, q);
            }

            template <class V_> void BreadthFirst(V_& visitFunc, std::queue<std::shared_ptr<Node_>>& q) {
                for (const auto& arg : arguments_)
                    q.push(arg);
