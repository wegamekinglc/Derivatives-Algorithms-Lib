...
namespace Dal {
    ...
    void Node_::SetOrder(unsigned int order) {
        order_ = order;
    }

    unsigned Node_::Order() const {
        return order_;
    }

    double& Node_::Adjoint() {
        return adjoint_;
    }
    ...
    void LogNode_::Evaluate() {
        result_ = std::log(arguments_[0]->Result());
    }

    void LogNode_::LogInstruction() {
        std::cout << "y" << order_ << " = log(y"
                  << arguments_[0]->Order() << ")" << std::endl;
    }
    ...
    double Leaf_::GetVal() const {
        return value_;
    }

    void Leaf_::SetVal(double val) {
        value_ = val;
    }
    ...
    shared_ptr<Node_> Number_::Node() const {
        return node_;
    }

    void Number_::SetVal(double val) {
        std::dynamic_pointer_cast<Leaf_>(node_)->SetVal(val);
    }

    double Number_::GetVal() const {
        return std::dynamic_pointer_cast<Leaf_>(node_)->GetVal();
    }
    ...
    shared_ptr<Node_> Log(const Number_& arg) {
        return shared_ptr<Node_>(new LogNode_(arg.Node()));
    }
}
