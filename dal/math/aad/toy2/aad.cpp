...
namespace Dal {
    ...
    Number_::Number_(double val) : node_(new Leaf_(val)) {
        tape_.push_back(std::unique_ptr<Node_>(node_));
    }

    Number_::Number_(Node_* node) : node_(node) {}

    Node_* Number_::Node() const {
        return node_;
    }

    void Number_::SetVal(double val) {
        dynamic_cast<Leaf_*>(node_)->SetVal(val);
    }

    double Number_::GetVal() const {
        return dynamic_cast<Leaf_*>(node_)->GetVal();
    }
