...
namespace Dal {
    ...
    template <class P_>
    GetFirst_<P_> GetFirst(const P_&) { return GetFirst_<P_>(); }

    template <class P_>
    GetSecond_<P_> GetSecond(const P_&) { return GetSecond_<P_>(); }
    ...
    template <class B_, class D_>
    B_* GetShared(const std::shared_ptr<D_>& src) { return src.get(); }

    template <class A_, class R_>
    std::function<R_(A_)> AsFunctor(R_ (*func)(A_)) {
    ...
    template <class SRC_, class DST_>
    struct ConstructCast_ {
