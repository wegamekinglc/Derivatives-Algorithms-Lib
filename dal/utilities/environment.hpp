...
namespace Dal {
    ...
    namespace  Environment {
        ...
        template <typename F_>
        auto Find(const Environment_* env, F_& func) -> decltype(func(*env->Begin())) {
        ...
        class Base_: public Environment_ {
            ...
            struct MyIter_: IterImp_ {
                ...
                IterImp_* Next() const override {
                    return new MyIter_(all_, Dal::Next(me_));
                }
        ...
        class XDecorated_: public Environment_ {
            ...
            struct I1: IterImp_ {
                const Environment_* parent_;
                const Entry_& val_;
                I1(const Environment_* parent, const Entry_& val)
                    :parent_(parent), val_(val) {}
                bool Valid() const override { return true;}
                IterImp_* Next() const override {
                    return parent_ ? parent_->XBegin() : nullptr;
                }
        ...
        public:
            XDecorated_(const Environment_*& theEnv, const Entry_& val)
                :theEnv_(theEnv), val_(val) {
                parent_ = theEnv_;
                theEnv_ = this;
            }
            ~XDecorated_() override { theEnv_ = parent_;}

            IterImp_* XBegin() const override {
                return new I1(parent_, val_);
            }
        };
    }
}
