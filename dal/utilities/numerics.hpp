...
namespace Dal {
    ...
    auto Accumulate(const C_& src, const OP_& op) {
        using val_type = decltype(op(0.0, *src.begin()));
        return std::accumulate(src.begin(), src.end(), val_type(0), op);
    }

    template <class C_>
    auto Accumulate(const C_& src) {
        return Accumulate(src, Plus<typename C_::value_type>);
    }

    template <class C1_, class C2_>
    auto InnerProduct(const C1_& src1, const C2_& src2) {
    ...
    namespace Vector {
        template <class T_>
        Vector_<T_> L1Normalized(const Vector_<T_>& base) {
        ...
        template <class T_>
        Vector_<T_> L2Normalized(const Vector_<T_>& base) {
            using val_type = typename Vector_<T_>::value_type;
            auto func = [](val_type x, val_type y) {
                return x + y * y;
            };
        ...
        template <class T_>
        Vector_<T_> Centralized(const Vector_<T_>& base) {
            using val_type = typename Vector_<T_>::value_type;
            size_t n = base.size();
            auto mean = Accumulate(base) / n;
            auto func = [&mean](val_type x) {
                return x - mean;
            };
        ...
        template <class T_>
        auto Covariance(const Vector_<T_>& src1, const Vector_<T_>& src2) {
        ...
        template <class T_>
        auto Correlation(const Vector_<T_>& src1, const Vector_<T_>& src2) {
            REQUIRE(src1.size() == src2.size(), "src1 size is not equal to src2 size");
