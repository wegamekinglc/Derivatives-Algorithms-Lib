...
template<class T_>
inline bool IsZero(const T_ &x) { return x < dal::EPSILON && -x < dal::EPSILON; }

template<class T_>
inline bool IsPositive(const T_ &x) { return x >= dal::EPSILON; }

template<class T_>
inline bool IsNegative(const T_ &x) { return x <= -dal::EPSILON; }
...
template<class T_>
inline T_ Max(const T_ &a, const T_ &b) { return a > b ? a : b; }

template<class T_>
inline T_ Min(const T_ &a, const T_ &b) { return a < b ? a : b; }
...
Handle_<T_> handle_cast(const std::shared_ptr<U_> &src) {
