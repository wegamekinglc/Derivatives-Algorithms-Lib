...
├── src/
│   ├── bindings/
│   │   └── dal_bindings.cpp  # pybind11 binding module
│   └── dal/
...
The Python bindings are generated by pybind11 from a single binding module. The build process:

1. **CMake** configures the build, locates DAL C++ libraries, and fetches pybind11
2. **C++ compiler** builds `_dal.cpython-*.so` extension module from `dal_bindings.cpp`
