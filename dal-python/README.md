...
2. Installs all dependencies (pytest, numpy, setuptools)
...
**Note:** The current SWIG binding only supports read access to matrix elements via `matrix(i, j)`. Use the constructor's fill value parameter to initialize all elements.
...
├── swig/
│   ├── dal.i              # Main SWIG interface
│   ├── date.i             # Date_ bindings
│   ├── models.i           # Model bindings
...
The Python bindings are generated by SWIG from interface files in `swig/`. The build process:

1. **CMake** configures the build and locates DAL C++ libraries
2. **SWIG** generates `dal_wrap.cpp` and `dal.py` from `.i` files
3. **C++ compiler** builds `_dal.cpython-*.so` extension module
4. **scikit-build-core** packages everything into an installable wheel

The hand-written Python code in `src/dal/` provides:
- `__init__.py` — Re-exports all SWIG-generated symbols
