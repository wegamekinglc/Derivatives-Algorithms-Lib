...
|---|---|---|---|
| Percent authoring must be adapted to canonical durable decimals outside the persisted draft | `POST /api/curve-lab/quote-canonicalizations` in `dal-web/backend/app/routers/curve_lab.py`; closed request/response and registry contracts in `dal-web/backend/app/schemas/curve_lab.py`; conversion in `dal-web/backend/app/services/quote_canonicalization.py` | `canonicalizeCurveLabQuote` in `dal-web/frontend/src/api/client.ts` and `CurveLabQuoteAuthoring.tsx` keep display input separate from canonical draft bytes | `test_curve_lab_api.py`, `test_curve_lab_quote_replay.py`, `curve_lab_quote_authoring.test.tsx`, and `api_client.test.ts` |
...
| Display preference is presentation state, not durable financial identity | Durable schemas persist only canonical raw/normalized values and registry-derived axes | `CurveLabQuoteAuthoring.tsx` owns the current display convention while the workspace draft receives canonical values | Frontend authoring tests verify convention switching and canonical request/response behavior |
...
This disposition approves a verification candidate only. It does not authorize
merging PR #265; DAL-23 independent verification, the documentation/CHANGELOG
decision, and DAL-18 final re-review remain separate gates.
