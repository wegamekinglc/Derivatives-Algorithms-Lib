...
- 新增 Excel 工作表函数以外的输出面变更、或改变任何现有函数签名/输出形状。
...
- **S14 payoff 闸门与接收器（B2 修复，写死）**：`ContainsPayoff` 扩展识别 `NodeExercise_`（`EXERCISE` 构成 payoff，`InvalidScriptStructure` 闸门对 EXERCISE-only 产品放行）。payoff 接收器规则：含 `PAYS` 的产品沿用现有 payoff 变量作持有侧累积器，驱动层在行权路径上以 `h_k` 覆盖；**EXERCISE-only（无变量）产品**的 payoff 索引取"无接收器"哨兵——LSMC 驱动自行聚合路径 payoff（行权 → `h_k·df`，从未行权 → 0），`payoff_index` 诊断沿用现有"无变量则省略"的输出守卫，`PayOffIdx` 提取不适用于该哨兵路径。
...
- **S16 错误定位契约**：本特性新增的全部错误（`UnsupportedExerciseDate`、`UnsupportedExerciseNesting`、`DuplicateExercise`、`InvalidExerciseCondition`、`UnsupportedRsgForExercise`）必须携带输入行号/偏移/日期等定位上下文（复用 `SourceOrigin_`），格式对齐 FIX 先例并被测试锁定；`DuplicateExercise` 尤须覆盖"同日多行合并"场景的定位。
...
- **X2 AST 与节点渲染**：`NodeExercise_ : ActNode_`，子节点 = [价值表达式] + 可选条件节点，fuzzy 元数据对齐 `CompNode_`。Debugger 三套渲染全覆盖：JSON 节点形 `{"id","kind":"exercise","children":[value, condition?], …}`（判别键** `"kind"`**，`"type"` 被 Fix/Spot 观测子对象占用；平滑元数据字段形对齐 compare 节点既有嵌套），s-表达式文本标签与 ASCII 树优先级表各加一项（`dal-cpp/dal/script/visitor/debugger.hpp`）。
...
- **N5 贴现**：日期间折算用现有样本网格 numeraire 比率（`SampleDef_`），与 `PAYS` 折现口径一致；S3/S9 的递归在各自日期时点计价，PV 折算至评估日。
...
- 聚焦测试（`dal-cpp/tests/script/test_exercise_parse.cpp`）：文法接受/拒绝（含 `;eps` 条件选项、保留字冲突、嵌套、重复、过期行权日、mrg32/irn 拒绝、dangling-IF 专用错误）；全部新错误的定位信息断言（含同日多行合并的 `DuplicateExercise`）；Describe JSON 黄金值；legacy debug /1 拒绝并提示迁移。
...
- 验收标准：
  ...
  - Excel：`dal-excel/tests/test_script_settings.cpp` 新 key 解析、未知 key 硬错、`SCRIPTSIMULATION.EXPLAIN` 契约（portable）；
