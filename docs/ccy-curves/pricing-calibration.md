...
The same joint-calibration workflow is available through the dedicated
[Python interface](../python/README.md), with a runnable
[`007.xccy_joint_calibration.py`](../../dal-python/examples/007.xccy_joint_calibration.py).
...
reset-aware analytic case, and joint calibration. The separate scheduled
Linux and Windows benchmark workflow executes this target; it is not in the
paired base/head regression allowlist. Its role is execution-smoke and
reporting coverage rather than a regression threshold gate.

## See Also

- [Yield-curve construction](../yield-curves/construction.md) — curve declarations and single or
  multi-curve calibration.
- [Yield-curve Jacobian](../yield-curves/jacobian-risk.md) — Jacobian layouts and
  inverse-Jacobian risk.
- [Underdetermined search](../methodology/underdetermined_search.md) — exact and approximate
  solver behavior.
