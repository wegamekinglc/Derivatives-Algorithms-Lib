...
the solver vector. See [Log-discount curve](log-discount.md) for the exact boundary,
...
quotes are covered in [Underdetermined search](../methodology/underdetermined_search.md).
...
in [Generic joint quote DV01](joint-quote-risk.md#calibration-and-the-selected-solution).
...
Risk](jacobian-risk.md) before consuming it.
...
[generic joint quote-risk guide](joint-quote-risk.md) describes construction,
...
Inverse-Jacobian Risk](jacobian-risk.md). Exact solves populate it by
...
for a runnable version. See [Log-Discount Curve](log-discount.md) for the
...
- [Underdetermined search](../methodology/underdetermined_search.md) — the optimisation method
  that performs the calibration solve.
- [Cross-currency calibration](../ccy-curves/pricing-calibration.md) — extends this framework to a
  cross-currency basis curve.
- [AAD methodology](../methodology/aad.md) — supplies the analytic Jacobian used in calibration
  and the curve risk sensitivities.
- [Log-Discount Curve](log-discount.md) — scalar-generic log-DF interpolation and
  its pinned-anchor column layout.
- [Script Engine](../methodology/script_engine.md) — the preprocessing and evaluation pipeline for
