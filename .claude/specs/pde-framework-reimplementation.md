...
- **C5 - No `mutable`, no `volatile`** per `.claude/rules/code-style.md`. Because
...
- **FR20** - `docs/methodology/pde.md` is rewritten current-state-only for the new stack
  ...
  content), `.claude/agents/dal-performancer.md` (describes `pde_perf` as "European call
  via `FD1D_`" — factually wrong after PR-3), and the
  `.claude/skills/dal-unit-test-write/SKILL.md` worked examples that reference
...
- **Style** - `.claude/rules/code-style.md` (trailing-underscore classes, camelCase
  params, lowercase-no-separator filenames, `#pragma once`, no `mutable`/`volatile`) and
  `.claude/rules/unit-test-style.md` (`TEST` only, no fixtures, `ASSERT_*` preferred,
...
All names are lowercase with no separators per `.claude/rules/code-style.md`.
...
- [ ] All old PDE files and their tests are removed (FR19 grep clean over `dal-cpp/` and
      ...
      `.claude/agents/dal-performancer.md`, `.claude/skills/dal-unit-test-write/SKILL.md`)
...
- **PR-3** - Phases 4-5: rewrite `european_fd`, `pde_perf`, run and report the one-off
  ...
  (`docs/README.md`, `.claude/agents/dal-performancer.md`,
  `.claude/skills/dal-unit-test-write/SKILL.md`), add `CHANGELOG.md` entry.
