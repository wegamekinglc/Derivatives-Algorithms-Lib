...
following. File:line anchors are project-relative.
...
| 6 | **Silent zero if `Compile()` skipped.** `MCSimulation` does not call `Compile()`; if a caller passes `compiled=true` without it, `nodeStreams_` are empty → every path yields 0/init, no assertion. | both | 🟠 footgun | `simulation.hpp`, `event.cpp:127-155` |
...
| 10 | Minor: `NodeCollect_` vestigial (never produced by parser, no visitor specializes it); scratch `T_ x,y,z,t` (`compiler.hpp:379`) adds AAD tape noise; public API hardcodes `compiled=false`, Python/Excel don't expose the flag. | — | 🟢 | `node.hpp`, `compiler.hpp:379`, `dal-public/src/value.cpp:35,41` |
...
misuse). Call `product.Compile()` once in the `<double>` path (idempotent). Gate
on the full gcc/clang/msvc × AAD-backend matrix. Leave `<Number_>` default
`false`.
...
regenerate-and-use `MG_NodeType_enum` (#9); prune `NodeCollect_`. Expose
...
| `TestParity_CompileNotCalled_GuardsThrow` | #3 / #6 guards | 🔴 |
...
random `:eps` values, random schedules; each generated product runs through
both evaluators on the same seed; assert parity at `1e-8`; ~500 products/run;
print the seed on failure. This covers combinations no human authored — the
actual "un-breakable" guarantee.
