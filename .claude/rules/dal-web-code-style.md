...
- Its methods MAY be called directly from inside `async def` handlers and
  ...
  `to_thread`.
- Do NOT rewrite the store as async without an explicit decision. See
  `.claude/specs/dal-web-db-persistence.md` for the rationale (the app is
  C++-pricing-bound, not DB-bound; doubling the driver matrix was rejected).
...
- Async refactor spec: `.claude/specs/dal-web-async.md`
- Sync persistence spec: `.claude/specs/dal-web-db-persistence.md`
