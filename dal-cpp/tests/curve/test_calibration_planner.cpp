...
TEST(CalibrationPlannerTest, TestAugmentedPlanPreservesTraversalOriginsAndRawParameterOrder) {
    ...
    ASSERT_EQ(plan.candidateTrace_[0].disposition_, CurveKnotCandidateDisposition_::Value_::ADDED);
    ASSERT_EQ(plan.candidateTrace_[2].disposition_, CurveKnotCandidateDisposition_::Value_::DUPLICATE);
    ASSERT_EQ(plan.candidateTrace_[3].disposition_, CurveKnotCandidateDisposition_::Value_::FILTERED_NOT_AFTER_TODAY);
    ASSERT_EQ(plan.candidateTrace_[3].origin_.kind_, CurveKnotOriginKind_::Value_::INSTRUMENT_START);
    ASSERT_EQ(plan.candidateTrace_[4].origin_.instrumentInputIndex_, 0);

    ASSERT_EQ(plan.resolvedDeclaredNodes_.size(), 3);
    ASSERT_EQ(plan.resolvedDeclaredNodes_[0].date_, Date::AddMonths(today, 1));
    ASSERT_EQ(plan.resolvedDeclaredNodes_[1].date_, Date::AddMonths(today, 3));
    ASSERT_EQ(plan.resolvedDeclaredNodes_[2].date_, Date::AddMonths(today, 6));
    ASSERT_EQ(plan.resolvedDeclaredNodes_[2].origins_.size(), 3);
    ASSERT_EQ(plan.resolvedDeclaredNodes_[2].origins_[0].inputKnotIndex_, 0);
    ASSERT_EQ(plan.resolvedDeclaredNodes_[2].origins_[1].inputKnotIndex_, 2);
    ASSERT_EQ(plan.resolvedDeclaredNodes_[2].origins_[2].instrumentInputIndex_, 1);

    ASSERT_EQ(plan.freeParameters_.size(), 6);
    for (int i = 0; i < 3; ++i) {
        ASSERT_EQ(plan.freeParameters_[2 * i].date_, plan.resolvedDeclaredNodes_[i].date_);
        ASSERT_EQ(plan.freeParameters_[2 * i].component_, CurveFreeParameterComponent_::Value_::LEFT_FORWARD);
        ASSERT_EQ(plan.freeParameters_[2 * i + 1].component_, CurveFreeParameterComponent_::Value_::RIGHT_FORWARD);
    }
    ASSERT_EQ(plan.counts_.submittedKnots_, 3);
    ASSERT_EQ(plan.counts_.instrumentCandidates_, 4);
    ASSERT_EQ(plan.counts_.resolvedDeclaredNodes_, 3);
    ASSERT_EQ(plan.counts_.storageNodes_, 3);
    ASSERT_EQ(plan.counts_.freeParameters_, 6);
}
...
TEST(CalibrationPlannerTest, TestZeroRatePlanAddsSyntheticStorageAnchorOnly) {
    ...
    ASSERT_EQ(plan.storageNodes_.front().date_, today);
    ASSERT_EQ(plan.storageNodes_.front().origins_.size(), 1);
    ASSERT_EQ(plan.storageNodes_.front().origins_.front().kind_, CurveKnotOriginKind_::Value_::SYNTHETIC_ANCHOR);
    ASSERT_EQ(plan.freeParameters_.size(), 2);
    ASSERT_EQ(plan.freeParameters_[0].component_, CurveFreeParameterComponent_::Value_::ZERO_RATE);
}
...
TEST(CalibrationPlannerTest, TestExecutionIdentityUsesActualInputDefinitionAndLayout) {
    ...
    ASSERT_EQ(identity.identityVersion_, 1);
    ASSERT_EQ(identity.executionPolicy_, CurveKnotPolicy_::Value_::INPUT);
    ASSERT_EQ(identity.today_, spec.today_);
    ASSERT_EQ(identity.parameterization_, CurveParameterization_::Value_::LOG_DISCOUNT);
    ASSERT_TRUE(identity.logDfScheme_.has_value());
    ASSERT_EQ(*identity.logDfScheme_, LogDfScheme_::Value_::MIXED);
    ASSERT_EQ(identity.resolvedDeclaredDates_, spec.knotDates_);
    ASSERT_EQ(identity.storageDates_, spec.knotDates_);
    ASSERT_EQ(identity.freeParameters_.size(), 3);
    ASSERT_EQ(identity.freeParameters_.front().date_, spec.knotDates_[1]);
    ASSERT_EQ(identity.freeParameters_.front().component_, CurveFreeParameterComponent_::Value_::LOG_DISCOUNT_FACTOR);
    ASSERT_EQ(identity.counts_.resolvedDeclaredNodes_, 4);
    ASSERT_EQ(identity.counts_.storageNodes_, 4);
    ASSERT_EQ(identity.counts_.freeParameters_, 3);
}
...
TEST(CalibrationPlannerTest, TestZeroRateExecutionIdentityCarriesActualLogDfScheme) {
    ...
    ASSERT_TRUE(identity.logDfScheme_.has_value());
    ASSERT_EQ(*identity.logDfScheme_, LogDfScheme_::Value_::MIXED);
    ASSERT_EQ(identity.parameterization_, CurveParameterization_::Value_::ZERO_RATE);
    ASSERT_EQ(identity.storageDates_.front(), spec.today_);
}
...
TEST(CalibrationPlannerTest, TestExecutionIdentityRejectsNonInputSpec) {
    ...
    ASSERT_THROW(static_cast<void>(InspectCurveCalibrationExecutionIdentity(spec)), Exception_);
}
...
TEST(CalibrationPlannerTest, TestStructuredAnalyticEligibilityUsesStableReason) {
    ...
    ASSERT_EQ(report.issues_[0].reason_.Switch(), AnalyticIneligibilityReason_::Value_::DISCOUNT_TARGET_REQUIRED);
    ASSERT_EQ(report.issues_[0].group_, String_("single"));
    ASSERT_EQ(report.issues_[0].declarationIndex_, -1);
    ASSERT_EQ(report.issues_[0].instrumentIndex_, -1);
}
